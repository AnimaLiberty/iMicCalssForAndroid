...
public class AccountModel extends SuperModel {
    ...
    public void saveNote(Note note) {
        if (note.mPaths.size() == 0) {
            ...
            Utils.Toast("笔迹保存成功");
            // 保存笔迹索引
            Utils.saveNoteIndex(note.mTitle);
        }
     ...
     * @param videoCallback
     */
    public void getVideoList(final VideoCallback videoCallback) {
        AsyncThreadPool.getInstance().executeRunnable(new Runnable() {
            @Override
            public void run() {
                final List<File> videoFiles = new ArrayList<>();
                final File cacheDir = CurveModel.getInstance().getAppVideoDir();
                if (!cacheDir.exists()) {
                    Utils.Toast("缓存目录不存在");
                } else {
                    File[] files = cacheDir.listFiles();
                ...
                mHandler.post(new Runnable() {
                    @Override
                    public void run() {
                        videoCallback.onCallback(videoFiles);
                    }
                });
            }
        });
    ...
    public interface VideoCallback {
        void onCallback(List<File> data);
    }
